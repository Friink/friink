...
- Notifications use a 4-second adaptive unread-count polling transport with visibility/focus recovery; the full list refreshes while Notifications is open. The web surface provides All/Security views, unread-only filtering, per-item read tracking, inline request actions, and duplicate-safe important-event toasts.
