  ...
  return (
    <Modal title="Crop profile picture" onClose={onCancel} onBack={onCancel} backLabel="Back to profile picture" closeLabel="Cancel crop" className="profile-picture-crop-dialog" actions={
      <>
        ...
        <button className="button-primary" type="button" disabled={busy} onClick={onConfirm} aria-label="Upload profile picture" title="Upload profile picture"><i className={`fa-solid ${busy ? 'fa-spinner fa-spin' : 'fa-check'}`} aria-hidden="true" /><span>{busy ? 'Uploading…' : 'Upload'}</span></button>
      </>
    }>
