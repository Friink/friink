...
export function AppShell({ user, onLogout, logoutError, initialScreen = 'home', initialSearchQuery, profileUser, profilePosts, profileReplies = [], children, floatingBarContent, showTabs, showFloatingBar = true, onUserChange, profileStats, profileLikedPosts: profileLikedPostsProp, profileLikedPostsHasMore = false, profileLikedPostsLoading = false, onLoadMoreProfileLikedPosts, profileConnectionsBasePath, connectionsUsername, initialConnectionsFilter = 'all', initialHomeFilter = 'all', initialMessagesTab = 'all', initialSettingsTab = 'general', initialSavedSection = 'posts', profileTab = 'posts', onProfileTabChange }: AppShellProps) {
  ...
  const [sidebarCollapsed, setSidebarCollapsed] = useAppShellState(user.id, 'sidebarCollapsed', true);
  const [appearance, setAppearance] = useAppShellState<AppearanceMode>(user.id, 'appearance', 'system');
  const [accentColor, setAccentColor] = useAppShellState(user.id, 'accentColor', '#33aa55');
  const [activeScreen, setActiveScreen] = useState<Screen>(initialScreen);
  const [posts, setPosts] = useAppShellState<Post[]>(user.id, 'posts', initialPosts);
  const [profileLikedPosts, setProfileLikedPosts] = useAppShellState<Post[]>(user.id, 'profileLikedPosts', profileLikedPostsProp ?? []);
  const [homeInjectedPost, setHomeInjectedPost] = useAppShellState<Post | null>(user.id, 'homeInjectedPost', null);
  const [floatingDraft, setFloatingDraft] = useAppShellState(user.id, 'floatingDraft', '');
  const [floatingPostBusy, setFloatingPostBusy] = useAppShellState(user.id, 'floatingPostBusy', false);
  const [composeContext, setComposeContext] = useAppShellState<ComposeContext>(user.id, 'composeContext', { kind: 'post' });
  const [profileConnectionState, setProfileConnectionState] = useAppShellState<'self' | 'none' | 'requested' | 'following'>(user.id, 'profileConnectionState', profileUser ? 'none' : 'self');
  const [profileConnectionRequestId, setProfileConnectionRequestId] = useAppShellState<string | null>(user.id, 'profileConnectionRequestId', null);
  const [connectionActionBusy, setConnectionActionBusy] = useAppShellState(user.id, 'connectionActionBusy', false);
  const [profileBlockOpen, setProfileBlockOpen] = useAppShellState(user.id, 'profileBlockOpen', false);
  const [profileBlockBusy, setProfileBlockBusy] = useAppShellState(user.id, 'profileBlockBusy', false);
  const [profileSaved, setProfileSaved] = useAppShellState<boolean | null>(user.id, 'profileSaved', null);
  const [profileSaveBusy, setProfileSaveBusy] = useAppShellState(user.id, 'profileSaveBusy', false);
  const [professionalRegistration, setProfessionalRegistration] = useAppShellState<ProfessionalRegistration | null>(user.id, 'professionalRegistration', null);
  const [registrationModalOpen, setRegistrationModalOpen] = useAppShellState(user.id, 'registrationModalOpen', false);
  const [registrationInstitute, setRegistrationInstitute] = useAppShellState(user.id, 'registrationInstitute', '');
  const [registrationCredentialId, setRegistrationCredentialId] = useAppShellState(user.id, 'registrationCredentialId', '');
  const [registrationBusy, setRegistrationBusy] = useAppShellState(user.id, 'registrationBusy', false);
  const [registrationError, setRegistrationError] = useAppShellState<string | null>(user.id, 'registrationError', null);
  const [incomingRequests, setIncomingRequests] = useAppShellState<ConnectionRequest[]>(user.id, 'incomingRequests', []);
  const [outgoingRequests, setOutgoingRequests] = useAppShellState<ConnectionRequest[]>(user.id, 'outgoingRequests', []);
  const [notifications, setNotifications] = useAppShellState<NotificationItem[]>(user.id, 'notifications', []);
  const [unreadNotificationCount, setUnreadNotificationCount] = useAppShellState(user.id, 'unreadNotificationCount', 0);
  const [unreadMessageCount, setUnreadMessageCount] = useAppShellState(user.id, 'unreadMessageCount', 0);
  ...
  const [connectionsFilter, setConnectionsFilter] = useAppShellState<'all' | 'followers' | 'following' | 'requests'>(user.id, `connectionsFilter:${initialConnectionsFilter}`, initialConnectionsFilter);
  const [messagesTab, setMessagesTab] = useAppShellState<'all' | 'muted' | 'requests' | 'archived'>(user.id, `messagesTab:${initialMessagesTab}`, initialMessagesTab);
  const [settingsTab, setSettingsTab] = useAppShellState<'general' | 'profile' | 'account' | 'subscription' | 'privacy'>(user.id, `settingsTab:${initialSettingsTab}`, initialSettingsTab);
  const [notificationsTab, setNotificationsTab] = useAppShellState<'all' | 'security'>(user.id, 'notificationsTab', 'all');
  const [notificationsUnreadOnly, setNotificationsUnreadOnly] = useAppShellState(user.id, 'notificationsUnreadOnly', false);
  const [notificationActionBusyId, setNotificationActionBusyId] = useAppShellState<string | null>(user.id, 'notificationActionBusyId', null);
  const [controlPanelTab, setControlPanelTab] = useAppShellState<ControlPanelTab>(user.id, 'controlPanelTab', 'overview');
