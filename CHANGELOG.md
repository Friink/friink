...
- [api] The wiped `api/` folder now contains a clean FastAPI starter with a root `Hello, World!` endpoint, Neon/Postgres connection wiring via `DATABASE_URL`, and a reset script for rebuilding the staging database schema from scratch.
