...
- [auth/docs] Synchronized the failed-login throttling replacement across the
  design contract, active rules, authentication documentation, and dedicated
  `docs/failed-login-policy.md` contract. The documented policy is failures
  1–3 without cooldown, one minute at 4–5, five minutes at 6–8, and a capped
  fifteen minutes from failure 9 onward, with 24-hour inactivity expiry,
  password-reset clearing, and a hashed per-IP secondary throttle.
- [security] Explicitly kept device/session throttling out of scope for this
  phase; existing device recognition and risk-based OTP remain separate.
- [verification] Documentation now records the local implementation status,
  named focused tests, and the absence of staging/production changes.

## 2026-09-10T10:00:00Z

- [api/migrations] Removed the redundant `uq_plan_entitlement` declaration
  from `PlanEntitlement` and the unapplied subscriptions migration; the
  `(plan_id, entitlement_key)` composite primary key already enforces the
  required uniqueness and no longer creates Alembic schema drift.
- [verification] Confirmed staging and production remain at
  `20260909_0041`; no persistent database was changed. The local SQLite test
  harness cannot execute the PostgreSQL-specific Alembic migration chain.
...
- [api] The FastAPI backend includes SQLAlchemy/Postgres wiring via sync psycopg3 sessions, Alembic migrations, Neon Postgres support, auth/session and account lifecycle routes, idempotent deterministic security-event audit writes, staff controls, the admin-only manual subscriptions foundation with Free/Pro/Pro+ capability resolution and lazy expiry, unified post/quote/reply creation, private-profile visibility enforcement, follow requests/connections, notifications, OTP/email flows, focused tests, and Vercel entrypoint support. The subscriptions backend and security-event idempotency fix are live on staging; the subscriptions migration is aligned with the composite-primary-key model.
- [auth] Cross-account session-state scoping remains an open risk: the browser-origin coordination keys and BroadcastChannel are not yet namespaced per account slot. This was investigated but not reproduced with a second test account and has not been fixed.
- [auth] Security-event audit writes now run through a separate, exception-isolated transaction boundary on auth, refresh, login, logout, failed-login, staff, and subscription mutation paths. Audit failures are logged and absorbed without changing the primary response or token/session state.
- [release] Commit `5a8df2f38b45d216e6542229d2e4d8a9adda77c4` is present on both `origin/staging` and `origin/development`; it adds the shared-modal staff verification presentation. This records branch state only and is not a production-readiness declaration.
