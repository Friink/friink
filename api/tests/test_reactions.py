...
from app.models.post import PostLike, PostSave
...
def test_like_and_save_flow_notification_visibility_and_lists() -> None:
    ...
    try:
        ...
        assert anonymous_post.json()["saved_count"] == 0
        assert anonymous_post.json()["liked"] is None
        assert anonymous_post.json()["saved"] is None
        ...
        actor_save = client.post(f"/posts/{post_id}/save", headers=actor_headers)
        assert actor_save.status_code == 200, actor_save.text
        assert actor_save.json()["saved_count"] == 1
        assert client.post(f"/posts/{post_id}/save", headers=actor_headers).json()["saved_count"] == 1
        ...
        assert owner_post["saved_count"] == 1
        assert owner_post["liked"] is True
        assert owner_post["saved"] is False
        ...
        actor_saved = client.get("/posts/saved", headers=actor_headers)
        assert actor_saved.status_code == 200, actor_saved.text
        assert [item["id"] for item in actor_saved.json()["items"]] == [post_id]
        ...
        assert client.delete(f"/posts/{post_id}/save", headers=actor_headers).json()["saved_count"] == 0
        assert client.delete(f"/posts/{post_id}/like", headers=actor_headers).json()["like_count"] == 1
        ...
        with get_session_factory()() as session:
            ...
            session.execute(delete(PostSave).where(PostSave.user_id.in_(user_ids)))
