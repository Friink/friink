...
async def list_blocked(session: Session, actor: User, query: str, cursor: str | None, limit: int) -> BlockedUserListResponse:
    ...
    return BlockedUserListResponse(items=[BlockedUserResponse(id=user.public_id, username=user.username, display_name=(user.display_name if user.lifecycle_status == "active" else "Friink User"), profile_picture_url=profile_picture_url_for(user) if user.lifecycle_status == "active" else None, blocked_at=block.created_at, show_professional_badge=user.show_professional_badge if user.lifecycle_status == "active" else False) for block, user in rows], next_cursor=_encode_cursor(rows[-1][0].created_at, rows[-1][0].id) if has_more and rows else None)
