...
async def get_chat_context(session: Session, user: User, username: str) -> ChatContextResponse:
    ...
    conversation = session.execute(
        select(Conversation)
        .options(selectinload(Conversation.user_one), selectinload(Conversation.user_two), selectinload(Conversation.messages).selectinload(Message.media))
...
async def send_message_to_user(session: Session, user: User, username: str, content: str, client_message_id: str, media: list[tuple[str, str | None]] | None = None) -> tuple[MessageResponse, Conversation]:
    ...
    if other.id == user.id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="You cannot message yourself.")
    user_one_id, user_two_id = sorted((user.id, other.id), key=str)
    conversation = session.execute(select(Conversation).options(selectinload(Conversation.user_one), selectinload(Conversation.user_two), selectinload(Conversation.messages).selectinload(Message.media)).where(Conversation.user_one_id == user_one_id, Conversation.user_two_id == user_two_id)).scalar_one_or_none()
