...
@router.post("/conversations/{conversation_id}/reject", response_model=ConversationResponse)
async def reject_chat_request(conversation_id: uuid.UUID, current_user: User = Depends(get_current_user), session: Session = Depends(get_session)) -> ConversationResponse:
    return await reject_request(session, current_user, conversation_id)
