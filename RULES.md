...
- **What:** Chat uses authenticated REST endpoints for conversation discovery, conversation creation, message history, and message sending. A conversation may be created only when both users have accepted follow relationships in both directions; following remains directional elsewhere in the product. The web client polls the active conversation every 4 seconds through a transport interface; the transport pauses while the document is hidden and polls immediately on focus/visibility recovery.
- **Edge cases:** Message history is incremental and cursor-based, messages are deduplicated by server ID, server timestamps determine ordering, and sends include a client message ID so retries do not create duplicates. The composer is unavailable for access-control reasons only when the API confirms that the users lack a mutual accepted follow relationship; unrelated loading or transient API errors must not be presented as a chat-policy denial. The `Muted` and `Requests` tabs remain present for compatibility but have no backend behavior yet. The transport boundary must remain stable so a future WebSocket transport can replace polling without changing the chat UI.
- **Status:** Active
- **Platform:** Web/API
