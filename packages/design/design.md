...
- **Floating Bar Rail Rule**: The persistent `FloatingBar` is rendered inside a fixed full-width rail that is centered with flex alignment. The rail follows the main panel on desktop and uses the shared content gutter (`16px` on desktop and `8px` on mobile); the bar itself is fluid up to the shared `ContentBox` cap of `512px` and is centered with `margin: auto`. Its contextual composer therefore remains centered without bar-level viewport-positioning arithmetic.
...
- **Content Width**: Shell content boxes are fluid (`width: 100%`) and responsive to the available app panel, with a primary desktop cap of `512px` (`--space-content-max-width`) for logged-in app content. Avoid per-screen hardcoded page max-width rules for primary app content; the shared container owns this constraint.
- **Shared Inset Tokens**:
...
- **Fixed Sizing & Positioning**:
  ...
  - Lives in a fixed `.floating-bar-rail` that centers the bar with `display: flex` and `justify-content: center`. The bar uses `width: 100%`, `max-width: var(--space-content-max-width)` (512px), and `margin: 0 auto`; the rail owns only the desktop sidebar boundary and responsive horizontal gutter.
