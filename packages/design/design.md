...
- **Floating Bar Rail Rule**: The persistent `FloatingBar` is rendered inside a fixed rail covering the same main-panel area as `ContentBox`: full viewport width on mobile, and from the desktop sidebar edge to the viewport edge on desktop. The rail uses the shared content gutter (`16px` on desktop and `8px` on mobile); the bar itself is fluid up to the shared `ContentBox` cap of `512px` and is centered with flex alignment and `margin: auto`. Its contextual composer therefore aligns with the content surface after the side drawer.
