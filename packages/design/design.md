...
- **Session Recovery**: A terminal refresh failure keeps the failed slot as context and shows recovery in the shared lifecycle card. A “Choose another remembered account” control expands an inline list using safe avatar and username metadata; it must not open a modal or automatically attempt another identity. Recovery actions share one centered width: paired actions use equal columns, while account choice occupies the full row so both rows align as one group. The selected account row shows progress while its coordinated refresh runs. Failures remain in the recovery card with an inline error; success alone activates the selected account. This recovery flow is separate from the existing active-logout/deactivation fallback.
