...
- **Floating Bar Rail Rule**: The persistent `FloatingBar` is rendered inside a fixed viewport-width rail that is centered with flex alignment. The rail uses the shared content gutter (`16px` on desktop and `8px` on mobile); the bar itself is fluid up to the shared `ContentBox` cap of `512px` and is centered with `margin: auto`. Its contextual composer therefore remains centered in the viewport without sidebar-positioning arithmetic.
