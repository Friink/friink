...
- **`Composer`**:
  ...
  - Post media controls: The post composer action menu's `Add media` item opens a multiple-image picker for up to eight client-side images. Selected images appear as a horizontally scrollable thumbnail strip between the `+` control and character count in the expanded composer toolbar. Selecting a thumbnail opens the shared `Modal` for preview, crop, or delete. Files remain local until Post is pressed, then upload through the authenticated submit flow. Failed submission preserves the text and attachments for retry; successful submission resets them. Published media is rendered by the shared Post Media Gallery contract below.
...
- **Layout**: One image uses a portrait `4:5` frame on compact screens and a responsive desktop frame capped at `min(70vh, 38rem)` so it remains visible above the floating composer. Two and four images use an even square grid. Three images use one larger left image with two stacked right images. Five through eight images show the first four positions with a `+N` overlay for the remaining count.
- **Surface**: The gallery uses the shared line/background tokens, a small platform radius, a compact gap, and full-width mobile bleed aligned to the content inset. Images use `object-fit: cover` and lazy loading after the first image.
