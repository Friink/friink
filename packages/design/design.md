...
- **Floating Bar Rail Rule**: The persistent `FloatingBar` follows the same centered content rail as `ContentBox`. It uses the main panel's sidebar-relative positioning, sits inset horizontally by the shared content gutter (`16px` on desktop and `8px` on mobile), and uses the `ContentBox` max width minus both gutters (`480px` at the 512px desktop cap). Its contextual composer therefore remains aligned with the primary content edges at every viewport width.
...
- **Fixed Sizing & Positioning**:
  ...
  - Follows the centered `ContentBox` rail with the same sidebar-relative desktop positioning and horizontal gutter. At the 512px desktop content cap, the bar's effective maximum width is 480px after the two 16px gutters; on mobile it remains fluid with 8px gutters.
