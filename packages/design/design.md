...
- **Deployment-Neutral Database Connection Rule**: Database connection management is infrastructure configuration, not product behavior. The API must use environment-configurable SQLAlchemy pool settings that work with Neon and a future Ubuntu-hosted PostgreSQL deployment. Neon Free uses conservative limits; Ubuntu may use a larger pool based on worker count and PostgreSQL `max_connections`. Do not encode Neon-only branching in application features.
