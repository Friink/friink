...
**Status:** Active
**Last edited:** 2026-09-22T10:38:57Z
...
- **Profile Composer Rule**: Profile pages do not render the standalone new-post composer, but selecting Reply or Quote on a profile post opens the shared contextual composer in the floating bar. The composer references the selected post and uses the same submit behavior as feed and post-detail surfaces.
...
- Conversation rows render an avatar-only profile link, one display-name title, and a secondary state line. Unread rows show `New message` or an unread count; the user's latest outgoing message shows `Sent`, `Delivered`, or `Seen`; otherwise the latest incoming preview is shown. Muted and Archived are represented by their tabs, not row labels. Mute/archive row actions use borderless icon-button geometry with transparent default surfaces and accent hover/focus/active treatment.
