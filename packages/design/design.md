...
- **Profile Picture Rule**: Settings > Profile includes an optional profile-picture picker with a circular preview, explicit choose/upload controls, and visible loading/error feedback. Uploads remain disabled until a file is selected; the existing default avatar remains the fallback when no picture URL exists. The preview may be optimistic while an upload is in progress, but must restore the last server-confirmed image if upload or confirmation fails. Upload failures must identify the failed stage (API start, R2 transfer, or API confirmation) and include an actionable configuration or session hint when the failure is environment-related.
