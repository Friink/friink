...
- **Direct Chat Scroll Rule**: The username-scoped conversation page uses the document viewport as its vertical scroll surface. The participant header and message history must not create a nested message-only scrollbar; the fixed contextual composer remains clear of the last message through shared page spacing.
