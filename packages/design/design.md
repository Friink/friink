...
- Saved uses `/saved/posts` and `/saved/profiles`; `/saved` redirects to `/saved/posts`. Posts contains the current user's private saved posts, while Profiles contains private saved profile rows. Unavailable saved profiles remain removable neutral rows without links or profile details. The Saved `Posts`/`Profiles` tabs are rendered at the shell level so they span the main panel; content remains inside the shared capped content box.
