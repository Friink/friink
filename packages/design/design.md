...
- **Direct Chat Scroll Rule**: The username-scoped conversation page uses the document viewport as its only native vertical scroll surface for message history. The participant header remains fixed below the global top bar and aligned to the centered `ContentBox` chat column; the route must not create a nested message-only scrollbar or widen beyond the shared content cap. The fixed contextual composer remains clear of the last message through shared page spacing.
