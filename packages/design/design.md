...
- **Direct Chat Scroll Rule**: The username-scoped conversation page uses the document viewport as its vertical scroll surface. The participant header remains sticky below the global top bar while the message history scrolls with the page; the route must not create a nested message-only scrollbar. The fixed contextual composer remains clear of the last message through shared page spacing.
