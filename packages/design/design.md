...
- **`Composer`**:
  ...
  - Chat visual mode: Chat uses the same multiline editor and mention-input variant as post composition so the shared embedded layout remains consistent. Chat intentionally does not inherit the post-only `256` character limit, live counter, or post-media upload flow until chat-message media and length contracts are defined.
