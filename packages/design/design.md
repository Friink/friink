...
- Saved uses `/saved/posts` and `/saved/profiles`; `/saved` redirects to `/saved/posts`. Posts contains the current user's private saved posts, while Profiles is reserved for future profile saving. The Saved `Posts`/`Profiles` tabs are rendered at the shell level so they span the main panel; feed content remains inside the shared capped content box.
