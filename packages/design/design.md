...
- **TopBar Preview**: `TopBar` (`web/components/top-bar.tsx`) is the functional signed-in top-bar prototype. It uses a full-viewport inner rail with shared edge padding and a leading/context/actions grid. Its theme-aware full lockup uses `logoFullBlack.svg` on light surfaces and `logoFullWhite.svg` on dark surfaces, links Home, renders at 40px high, and has a 16px gap after the hamburger or Back control. Desktop centers the current title. At mobile widths (`max-width: 767px`), Home omits the title and shows the full logo; regular contextual screens hide the logo and show the title beside Back, with actions aligned to the right. The search route retains its Back/search/actions layout. Hamburger, Back, Search, Chat, Notifications, Filter, Actions, and expanded-search control glyphs are 24px inside 40px hit areas. The centered desktop title and expanded-search input text use 20px; ActionMenu row glyphs use 16px. Chat and Notifications show actual unread counts in top-right accent pills: 1–9 display directly and larger counts display `9+`. Pills are 16px high with 12px text, a 1px `--color-paper` border, and the shared accent background. The search-filter active state retains its separate presence dot. TopBar icon controls use `--color-sidebar-active` for hover and keyboard-focus backgrounds in both themes while retaining accent-colored icons. The preview keeps the existing `Header` and `NavigationBar` mounted but hides them with shared CSS.
