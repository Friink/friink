...
- **Chat Composer Availability Rule**: The chat composer is disabled for access-control reasons only after the API confirms that the two users do not have mutual accepted follows. Loading or unrelated transient API errors must not be represented as a policy-based unavailable state.
