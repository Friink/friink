...
- **Password Change Rule**: Settings > Account includes a password-change row with Current password, New password, and Confirm new password fields. The action is disabled until the new password meets the signup password rules and both new-password fields match; the backend remains authoritative and requires the current password before replacing the stored hash. A successful change keeps the current session active and clears the password fields.
