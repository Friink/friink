...
- **Direct Chat Scroll Rule**: The username-scoped conversation page uses its centered `ContentBox` chat column as one contained native vertical scroll surface. The participant header remains sticky at the top of that column below the global top bar; the route must not create a separate message-only scrollbar or widen beyond the shared content cap. The fixed contextual composer remains clear of the last message through shared page spacing.
