...
- Search control contract: the shared TopBar search icon is available on every
  signed-in screen except the search route. Contextual screens place it beside
  the Actions menu; the search route keeps Back and Actions and lets the search
  field fill the middle space between them. On mobile, an opened search
  surface expands to the viewport gutter on non-search routes; the search route
  retains the Back-to-Actions middle slot. The persistent search field on the
  search route does not force the optional suggestions dropdown open; field and
  dropdown visibility are separate states.
...
- **TopBar Preview**: `TopBar` (`web/components/top-bar.tsx`) is the functional signed-in top-bar prototype. It uses a full-viewport inner rail with shared edge padding and a leading/context/actions grid. Its theme-aware full lockup uses `logoFullBlack.svg` on light surfaces and `logoFullWhite.svg` on dark surfaces, links Home, renders at 38px high inside a 40px control rail, and has a 16px gap after the hamburger or Back control. Desktop centers the current title. At mobile widths (`max-width: 767px`), Home omits the title and shows the full logo; regular contextual screens hide the logo and show the title beside Back, with actions aligned to the right. The search route retains its Back/search/actions layout. Hamburger, Back, Search, Chat, Notifications, Filter, Actions, and expanded-search control glyphs are 20px inside 40px hit areas. Page titles use 18px; expanded-search input text uses 20px. ActionMenu row glyphs use 16px. Chat and Notifications show actual unread counts in top-right accent pills: 1–9 display directly and larger counts display `9+`. Pills are 16px high with 12px white vertically centered text and no border in either theme. The search-filter active state retains its separate presence dot. TopBar icon controls use `--color-sidebar-active` for hover and keyboard-focus backgrounds in both themes while retaining accent-colored icons. The preview keeps the existing `Header` and `NavigationBar` mounted but hides them with shared CSS.
...
- **Scrollbar Treatment**: The shared app stylesheet styles native scrollbars globally with thin rounded muted-neutral thumbs, transparent tracks, and a slightly stronger neutral hover treatment. Light theme uses the existing muted/line blend; dark theme uses darker neutral `#666666` and hover `#7a7a7a`. Scrollbars must not use the brand/accent color. Do not replace native scrolling with a JavaScript scrollbar component; wheel, keyboard, touch, accessibility, and reduced-motion behavior remain browser-owned.
  - Exception: the tablet/desktop SideDrawer keeps native vertical scrolling but hides its scrollbar indicator to avoid a transient scrollbar during width animation.
- **Direct Chat Scroll Rule**: The username-scoped conversation page uses the document viewport as its only native vertical scroll surface for message history. The participant header remains fixed below the global top bar and aligned to the centered `ContentBox` chat column; on desktop, its opaque background spans the full main panel so messages cannot show through beside the capped column. The route must not create a nested message-only scrollbar or widen beyond the shared content cap. Shared shell bottom padding does not compound the message-list reservation, and the fixed contextual composer remains clear of the last message with a consistent `1rem` gap across viewport sizes.
- **Feed Surface Treatment**: `FeedPost` remains a divider-separated, flat feed item. Its shared hover state uses a quiet surface shift, and `ContentBox` does not add a visible outline or frame line. Individual posts must not gain heavy card borders as a route-only styling fix.
- **Post Media Treatment**: `PostMediaGallery` uses a stable single-image frame, balanced two-item layout, lead-image three-item layout, and four-item overflow grid. Media opens in a dedicated full-viewport lightbox with previous/next/close controls, keyboard navigation, and a counter; it preserves layout space while loading and does not autoplay audio.
- **Chat Media Treatment**: `ChatMediaGallery` uses the same bounded media language as posts while keeping message bubbles compact: one image preserves its aspect ratio in a contained frame, two to four images use balanced tiles, and the fourth tile carries a `+N` overflow indicator. Selecting a tile opens the shared full-viewport lightbox with previous/next/close controls, keyboard navigation, and a counter.
...
- **Subscription Disclosure**: Subscription surfaces use explicit plan labels (`Free`, `Pro`, `Pro+`) and assignment states (`Active`, `Expired`, `Revoked`); indefinite access is labeled `No expiration`. Manual access is described as granted or activated access, never as a purchase. Public plan and professional badges are opt-in and hidden by default. The professional-badge preference is revealed under General only when `How I use Friink` is `For professional networking`; it is hidden for `For personal connection`, appears immediately next to the user's displayed name on the public profile, and does not itself grant professional status, directory access, or paid entitlement.
- **Manual Subscription UX Contract**: Planned staff access changes use `Grant access`, `Change access`, and `Return to Free` with a confirmation summary for the resulting plan, effective date, expiry, and replacement effect. Staff see assignment history and audit reasons; users see effective plan, status, expiry, and concise lifecycle feedback. Unimplemented paid features remain `Coming soon` in the plan comparison until they have a real entitlement-gated surface.
...
- **Search Scope**: Search uses the shared `Tabs` strip at the application-shell level for the mutually exclusive `All`, `People`, `Posts`, and `Messages` scopes. Scope selection remains URL-addressable; the strip spans the main panel while result content remains inside the shared capped content box. Search must not introduce search-specific tab styling or behavior.
...
- **OTP Configuration Surface**: OTP behavior is controlled by the API-owned `OTP_ENABLED` master setting. Development may set it to `false`; staging and production must keep it enabled. The API refuses to start in production with the master switch disabled. The frontend must follow API response state and must not render an OTP step when verification is disabled. See [`docs/deployment.md`](../../docs/deployment.md) for the environment release policy.
...
- **Remembered Accounts**: The account caret lists server-provided device slots in their returned order, followed by `Add account`. The active account remains usable until a new account session and slot are fully established.
...
- **Session Recovery**: A terminal refresh failure keeps the failed slot as context and shows recovery in the shared lifecycle card. “Choose another remembered account” opens the shared modal, keeping the recovery page height fixed. The current account is first, followed by other accounts in most-recent order. Each row uses safe avatar and username metadata; only an explicit choice triggers a refresh. Recovery actions share one centered width, with paired actions in equal columns. The selected modal row shows progress while that slot's coordinated refresh runs. Failures remain in the modal with an inline error; success alone activates the selected account. This recovery flow is separate from the existing active-logout/deactivation fallback.
...
2. **SideDrawer (Personal Identity & Network)**:
   ...
   - The account caret opens the shared `ActionMenu`: all remembered accounts in server-provided order, then Add account.
...
- Saved uses `/saved/posts` and `/saved/profiles`; `/saved` redirects to `/saved/posts`. Posts contains the current user's private saved posts, while Profiles contains private saved profile rows. Saved Profile rows render the ProfileCard identity once, with no duplicate floating title/handle copy; available rows expose Follow/Following plus a More menu for Remove from saved and Block user, while unavailable rows retain only their removal control. The trailing removal control is a visible star with a slash. Unavailable saved profiles remain removable neutral rows without links or profile details. The Saved `Posts`/`Profiles` tabs are rendered at the shell level so they span the main panel; content remains inside the shared capped content box.
- Directory uses `/directory` with shell-level `Tabs` for `All`, `Professionals`, and `Friink Registered`; its result content reuses `PageSurface`, `ListRow`, and `ProfileCard` inside the capped content box. Rows show Professional/Friink Registered badges beside the profile name, About text below, and contextual icon-only actions on the right in this order: Chat, Follow/Unfollow, and More. Rows link to public profiles and stack secondary content on narrow screens.
...
- **Saved Feed**: A preset saved-post view containing only saved posts at `/saved/posts`, alongside Saved Profiles at `/saved/profiles`.
...
- **Button Link Rule**: `.button-primary` and `.button-secondary` remove
  browser-default link underlines so anchors styled as actions match button
  controls in auth, recovery, modal, and settings surfaces.
...
- **Variants & Action Rules**:
  - **Self-Profile Variant** (`isOwnProfile = true`): Renders the **Edit** action button (`.button-primary.profile-action-edit`, icon `fa-pen-to-square` + text "Edit", right-aligned) and routes to Settings > Profile.
  - **Other-User / Dummy Profile Variant** (`isOwnProfile = false`): Renders the connection action as `.button-primary.profile-connection-action` with icon and label on one line; Message remains a compact `.button-primary.icon-button` control and routes to `/{username}/chat` when activated. The shell-owned contextual NavigationBar menu supplies a star-icon Save profile action, changing to Remove from saved after saving, alongside moderation actions such as Block.
...
### 6. TopBar (`web/components/top-bar.tsx`) & NavigationBar (`web/components/navigationbar.tsx`)
- **Desktop `Header`**:
  ...
  - Home mode right actions: Search button (`fa-magnifying-glass`) opens an inline search input with the submit icon before the close (`fa-xmark`) button. Chat link (`fa-regular fa-envelope`) sits between Search and Notifications, routes to `/chats`, and shows a green dot whenever any conversation has an unread message. Search, Chat, and Notifications use equal action cells with the newer hover treatment. The floating suggestions dropdown uses the shared `ContextualDropdown`, and the notification bell opens the unread notification dropdown.
  - Search submission: Clicking the right-side search button or pressing Enter navigates to `/search/{searched-string}`.
  - Notifications bell button (`fa-regular fa-bell`) matches search icon height, stays aligned at the right edge of the header actions, and opens a floating dropdown anchored to the bell. The dropdown shows unread notifications only, is empty when unread count is zero, measures its rendered top edge to calculate the remaining viewport height, shows at most eight rendered rows at once, supports scrolling only in the list after the eighth row while keeping the footer visible, shows an `x new` pill using the actual unread count (`99+` when above 99), and provides an `All Notifications` link to `/notifications`. The bell shows a small green dot only when unread notifications exist; the dot is hidden at `0`. Header spacing must reserve room so the indicator is not clipped at the viewport edge.
...
- **Fixed Internal Layout Order**:
  ...
  - The Profile identity row is excluded from that active-row gray fill. In the expanded drawer, account-switcher hover, keyboard-focus, and open states use `--color-sidebar-active` while keeping the icon accent-colored; collapsed account-switcher surface styling remains unchanged.
  - At viewport widths of 768px and above, a non-touch pointer temporarily expands a collapsed drawer on hover. It returns to ribbon width on pointer exit; the drawer overlays the page while its reserved layout width, top bar position, and persisted hamburger state remain collapsed. Touch pointers and mobile widths do not trigger hover expansion.
  - With no saved drawer cookie, the desktop/tablet drawer's first render is collapsed. A saved expanded preference is applied after the cookie is read; mobile always starts collapsed.
  - The tablet/desktop drawer's native vertical scrolling remains available while its scrollbar indicator is hidden.
  ...
  4. Footer actions (`.sidebar-footer`): Settings (`fa-gear`) and Log out (`fa-right-from-bracket`). Account switching and Add account live in the profile-card account menu. The switcher remains a device-session convenience, not an account-linking surface.
- **Responsive Behavior**:
...
- **`Composer`**:
  ...
  - Chat visual mode: Chat uses the same multiline editor, mention-input variant, and media attachment strip as post composition so the shared embedded layout remains consistent. Chat does not inherit the post-only `256` character limit, but it does reuse the post-media image preparation flow with an eight-image limit.
  - Chat message limit: Chat always shows a `count/2048` counter and enforces the 2,048-character limit; it does not inherit the post-only `256` limit, while its image attachments reuse the post-media upload preparation flow.
  ...
  - Draft behavior: Composer text is held in the owning screen's in-memory state only. It is not persisted or restored through browser storage, and leaving the screen clears the composer when it unmounts. The mention editor exposes its empty state explicitly so its placeholder returns after all text is deleted, including when the browser leaves an empty contenteditable node.
  - Media controls: The composer action menu's `Add media` item opens a multiple-image picker for up to eight client-side images. Selected images appear as a horizontally scrollable thumbnail strip between the `+` control and character count in the expanded composer toolbar; thumbnails can be reordered before submission. Selecting a thumbnail opens the crop tool directly with a `3:5` frame, Reset and Apply controls, and previous/next overlay arrows when multiple images are attached. While the post or chat-media request runs, the send action is disabled and shows the spinner animation. Files remain local until submission, then upload through the authenticated content flow. Failed submission preserves the text and attachments for retry; successful submission resets them. Posts use the shared Post Media Gallery; chat messages render the attached images inline in the bubble.
...
- **Layout**: Galleries use predictable responsive arrangements: one image is shown in a natural-ratio frame constrained by content width and maximum height; two images use a balanced grid; three images use a larger lead image beside two smaller images; and four or more images use a four-tile grid. The fourth tile exposes a `+N` overflow affordance, while every associated image remains available in the viewer.
- **Surface**: The gallery uses shared line/background tokens, a small platform radius, and a `0.35rem` grid gap with full-width mobile bleed aligned to the content inset. Multi-image tiles use `object-fit: cover`; single-image previews preserve the complete image with `object-fit: contain`. Images load eagerly only for the first image and lazily thereafter.
- **Viewer**: Activating any image opens a dedicated full-viewport lightbox rendered above the app shell. The lightbox contains the image within a responsive contained stage, shows a `current / total` counter, provides close/previous/next controls, supports Escape and left/right arrow keys, closes on backdrop activation, and locks document scrolling while open.
...
- **Layout**: Horizontal tab row (`.tabs__pill`, `role="tab"`) with selected text in bold white and the active-tab underline using the user-configurable `--color-accent`. Tab labels use `0.8rem`; unselected labels remain `font-weight: 400`. The underline (`.tabs__indicator`) slides smoothly between tabs. The tab container has a subtle full-width bottom rule and selected tabs have no background fill. Top app tab strips are `1.98rem` tall and sit directly below `NavigationBar` without a gap. The tab row scrolls horizontally when its items exceed the available width, with the scrollbar hidden to preserve the sleek strip appearance. Settings, Profile, and Connections tabs use the same bold white selected-label treatment while retaining their existing accent underline.
...
- **In-App Button System** (`Button`):
  - The app has two action styles: `primary` (`.button-primary`) for the main action and `secondary` (`.button-secondary`) for supporting, reversible, or cancel actions. Primary uses a solid `--color-accent` surface with contrasting text; secondary uses a translucent accent surface with accent text. Both use `3rem` minimum height, `0.75rem 1.25rem` padding, `8px` radius, shared typography, focus, disabled, and loading behavior.
  - Text-only, icon-and-text, and icon-only content are compositions, not additional button types. Icon-and-text `.button-primary`/`.button-secondary` controls use the shared `0.5rem` gap; icon-only controls use `.icon-button`, with a minimum `2.75rem` square hit area and an accessible label. Semantic icon-only actions retain their primary/secondary treatment; neutral utility controls use the neutral `.icon-button` treatment.
...
- **Professional Badge Setting Rule**: General keeps `How I use Friink` as a private two-option preference. Selecting `For professional networking` reveals the subordinate setting `Show you are a professional on Profile`; selecting `For personal connection` hides or disables that subordinate setting. The professional badge is hidden by default, uses the shared settings-row/action-rail treatment, appears immediately next to the displayed name on the public profile, and controls profile presentation only.
...
- **Composition Rule**: `ListRow` supports an optional middle slot for secondary status content between the identity copy and the trailing action rail. Directory uses this slot for Professional/Friink Registered badges.
- **Profile Badge Rule**: `ProfileCard` owns the active-badge flags and renders only active reusable icon-only badges beside the profile name. Directory uses `fa-briefcase` for `Professional` and `fa-shield-halved` for `Friink Registered`; each uses the shared accent-colored compact pill treatment, exposes its full meaning through its hover and accessible label, and both may appear together.
